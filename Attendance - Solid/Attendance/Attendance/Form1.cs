...
namespace Attendance
{
    public partial class frmAttendance : Form
    {
        ...
        {
            InitializeComponent();
            refreshFormTimer.Interval = 1000 * 300; //refresh every 5 minutes
        ...
        {
            dgView2Load();
            //MessageBox.Show("Successfully Refresh 5 minutes!");
        }
        ...
        private void dgView2Load()
        {
            ...
            MySqlCommand cmd2 = new MySqlCommand("SELECT employees.employee_id,CONCAT(employees.first_name,' ',employees.last_name),attendanceepinoy.employee, attendanceepinoy.in_time_am, attendanceepinoy.out_time_am, attendanceepinoy.in_time_pm, attendanceepinoy.out_time_pm, employees.id FROM attendanceepinoy INNER JOIN employees ON attendanceepinoy.employee = employees.id  WHERE (DATE(attendanceepinoy.in_time_am)=CURDATE() OR DATE(attendanceepinoy.in_time_pm)=CURDATE()) AND (employees.employee_id != 'EMP001' AND employees.employee_id != 'EMP00999') ORDER BY attendanceepinoy.id DESC", connection);
            try
            {
                ...
                dgViewAttendance.Columns[1].HeaderCell.Value = "Employee";
                dgViewAttendance.Columns[1].Width = 110;
                dgViewAttendance.Columns[2].Visible = false;
                dgViewAttendance.Columns[3].HeaderCell.Value = "Time In AM";
                dgViewAttendance.Columns[3].Width = 110;
                dgViewAttendance.Columns[4].HeaderCell.Value = "Time Out AM";
                dgViewAttendance.Columns[4].Width = 110;
                dgViewAttendance.Columns[5].HeaderCell.Value = "Time In PM";
                dgViewAttendance.Columns[5].Width = 110;
                dgViewAttendance.Columns[6].HeaderCell.Value = "Time Out AM";
                dgViewAttendance.Columns[6].Width = 110;
                dgViewAttendance.Columns[7].Visible = false;
            } catch (Exception ex)
            {
                MessageBox.Show(ex.Message);
            }
                                        ...
                                        using (MySqlConnection cnn1 = new MySqlConnection(connectionString))
                                        {
                                            ...
                                            if (userCount > 0)
                                            {
                                                using (MySqlConnection cnUpdateTimeOut = new MySqlConnection(connectionString))
                                                {
                                                    try
                                                    {
                                                        string kumpleto = "Complete";
                                                        string qryUpdateTimeOut = "UPDATE attendanceepinoy SET out_time_am = '" + txtDateTime.Text + "',attendanceimpression = '" + kumpleto + "' WHERE (DATE(in_time_am) LIKE CURDATE()) AND employee LIKE '" + txtID.Text + "'";
                                        ...
                                        using (MySqlConnection cnn1 = new MySqlConnection(connectionString))
                                        {
                                            ...
                                            if (userCount > 0)
                                            {
                                                using (MySqlConnection cnUpdateTimeIn = new MySqlConnection(connectionString))
                                                {
                                                    try
                                                    {
                                                        string dikumpleto = "Incomplete";
                                                        MySqlCommand cmdInsert = new MySqlCommand("UPDATE attendanceepinoy SET in_time_pm = '" + txtDateTime.Text + "', attendanceimpression = '" + dikumpleto + "' WHERE (DATE(in_time_am) LIKE CURDATE() OR DATE(in_time_pm) LIKE CURDATE()) AND employee LIKE '" + txtID.Text + "'", cnUpdateTimeIn);
                                                        cnUpdateTimeIn.Open();
                                                        try
                                                        {
                                                            if (cmdInsert.ExecuteNonQuery() == 1)
                                            ...
                                            else
                                            {
                                                ...
                                                using (MySqlConnection cnInsertTimeIn = new MySqlConnection(connectionString))
                                                {
                                                    ...
                                                    {
                                                        string sMonth = DateTime.Now.ToString("MM");
                                                        string qryInsert = "INSERT INTO attendanceepinoy (employee,in_time_pm,monthnumber) VALUES ('" + txtID.Text + "','" + txtDateTime.Text + "','" + sMonth + "')";
                                                        MySqlCommand cmdInsert = new MySqlCommand(qryInsert, cnInsertTimeIn);
                                                        cnInsertTimeIn.Open();                                                        
                                                        try
                                                        {
                                                            dgView2Load();
                                        ...
                                        using (MySqlConnection cnn1 = new MySqlConnection(connectionString))
                                        {
                                            ...
                                            if (userCount > 0)
                                            {
                                                using (MySqlConnection cnUpdateTimeOut = new MySqlConnection(connectionString))
                                                {
                                                    ...
                                                    {
                                                        string kumpleto = "Complete";
                                                        MySqlCommand cmdInsert = new MySqlCommand("UPDATE attendanceepinoy SET out_time_pm = '" + txtDateTime.Text + "', attendanceimpression = '" + kumpleto + "' WHERE (DATE(in_time_am) LIKE CURDATE() OR DATE(in_time_pm) LIKE CURDATE()) AND employee LIKE '" + txtID.Text + "'", cnUpdateTimeOut);
                                                        cnUpdateTimeOut.Open();
                                                        try
                                                        {
                                                            if (cmdInsert.ExecuteNonQuery() == 1)
        ...
        private void txtCode_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            else if (e.KeyCode == Keys.Enter)
            {
                ...
                //MessageBox.Show("You do not need to press Enter, After inputting your attendance code press F1 for Time In AM, F2 for Time Out AM, F3 for Time In PM, F4 for Time Out PM", "Error Encountered:", MessageBoxButtons.OK, MessageBoxIcon.Error);
                //txtCode.Focus();
            }
        }
        ...
        {
            const string message = "Are you sure you want to close Attendance Monitoring Application Now?";
